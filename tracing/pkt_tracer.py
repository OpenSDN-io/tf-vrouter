...
def stp_handler(signum, frame):
    ...
    fd = open("trace_output", "w")
    for line in arr:
        pid = " ".join(line.split()).split(" ")[2]
        if pid not in fmap:
           indent[pid] = 0
           fmap[pid] = line
        else:
           fmap[pid] = fmap[pid] + line
        if '{' in line:
           indent[pid] += 1
        elif '}' in line:
           indent[pid] -= 1
...
class Probe(object):
    def __init__(self, probe_name, handler, ret_handler, event, ret_flag, org_flag, ret_event):
        ...
        self.org = org_flag
        self.ret_events_name = ret_event
    ...
    def _generate_program(self, args, largs):
        ...
        self.text += """
            ...
            int %s(struct pt_regs *ctx) {
                ...
                data.ret = 1;
                """ %(self.ret_events_name, self.ret_handler)
        if largs == 0:
            self.text += """
                %s.perf_submit(ctx, &data, sizeof(data));
                return 0;
            }""" %(self.ret_events_name)
        else:
            self.text += """
                int *val = trace_map.lookup_or_init(&data.pid, &zero);
                if(*val == 1)
    ...
    def ret_print_event(self, cpu, data, size):
    ...
    def attach_probe(self, bpf):
        ...
        bpf[self.ret_events_name].open_perf_buffer(self.ret_print_event)
...
"""
    ...
    def _create_probes(self):
        ...
        for line in f:
            line = line.strip()
            self.probes.append(Probe(line, "probe_handler_"+line, "ret_probe_handler_"+line, "events_"+line, self.return_flag, self.org, "ret_events_"+line))
