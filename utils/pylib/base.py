...
class Base(object):
    """
    Base class for launching vrouter and vtest and sending sandesh messages
    """
    logger = logging
    vr_args = {}
    vt_args = {}
    ...
    @classmethod
    def tearDown(self):
        """Add code to remove xml files created for each test if needed"""
        pass
    ...
    def vtest_ut_cleanup(self):
        """Api to cleanup files created after running vtest"""
        shutil.rmtree(self.get_test_file_path(), ignore_errors=True)

    @classmethod
    def get_sandesh_req_num(self):
        """Returns sandesh request number"""
        Base.sreq_num += 1
        return Base.sreq_num

    def replace_sandesh_obj_name(self, obj, file):
        """
        Replaces sandesh obj name with class name in xml file generated
        as pysandesh uses the derived class name to write the req.
        Eg: vif instead of vr_interface_req
        """
    ...
    def get_xml_proto_file_handle(filehandle):
        """Creates xml proto with file handle"""
    ...
    def parse_xml_field(self, file, field_name):
        """Parses and returns value of the field"""
        tree = ET.parse(file)
    ...
    def send_sandesh_req(self, obj_list, get_resp=False):
        """Sends sandesh request to vrouter"""
        obj_list_internal = []
        ...
        for obj in obj_list_internal:
            ...
            self.run_vtest_command(False, req_filename,
                                   resp_filename)
    ...
    def get_output_pcap_file(self, req_file):
        """Returns output pcap file for input pcap file"""
